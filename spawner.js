...
global.CanvCass = require("@cass-modules/CassieahExtras").CanvCass;

function monitorBlocking(
  delay = 5000,
  threshold = 100,

  onBlock = (blockTime, delta) => {
    console.warn(`⚠️ Event loop blocked for ${blockTime}ms`);
  }
) {
  let last = Date.now();
  setInterval(() => {
