...
const langData = {
  en_US: {
    ...
    "resort.buySuccess":
      "𝙲𝚘𝚗𝚐𝚛𝚊𝚝𝚞𝚕𝚊𝚝𝚒𝚘𝚗𝚜! 𝚈𝚘𝚞'𝚟𝚎 𝚙𝚞𝚛𝚌𝚑𝚊𝚜𝚎𝚍 𝚊 𝚛𝚎𝚜𝚘𝚛𝚝 𝚗𝚊𝚖𝚎𝚍 {resortName}!",
...
setInterval(() => {
  for (const [_userID, userResort] of userResorts.entries()) {
    ...
    userResort.cleanliness = Math.max(
      userResort.cleanliness - cleanlinessDecrease,
      0
    );
...
function loadUserData() {
  try {
    ...
    /**
     * @type {typeof import("@root/temp/user_resorts.json")}
     */
    const parsedData = JSON.parse(data);

    userResorts = new Map(
      parsedData.userResorts.map(([userID, userData]) => {
        const { lastUpgradeTime = 0, ...restData } = userData;
        return [
          userID,
          { ...restData, name: restData.name || "", lastUpgradeTime },
        ];
      })
    );
...
async function onCall({ message, getLang, args }) {
  ...
  // const resortimage = (
  //   await axios.get("https://i.imgur.com/kKP3G5t.png", {
  //     responseType: "stream",
  //   })
  // ).data;
  ...
  if (args[0] === "buy") {
    if (userResorts.has(senderID)) {
      return message.reply(
        "𝚈𝚘𝚞 𝚊𝚕𝚛𝚎𝚊𝚍𝚢 𝚘𝚠𝚗 𝚊 𝚛𝚎𝚜𝚘𝚛𝚝. 𝙸𝚏 𝚢𝚘𝚞 𝚠𝚊𝚗𝚝 𝚊 𝚗𝚎𝚠 𝚘𝚗𝚎, 𝚢𝚘𝚞 𝚌𝚊𝚗 𝚜𝚎𝚕𝚕 𝚢𝚘𝚞𝚛 𝚌𝚞𝚛𝚛𝚎𝚗𝚝 𝚛𝚎𝚜𝚘𝚛𝚝 𝚞𝚜𝚒𝚗𝚐 `resort sell`."
      );
    }
  ...
  if (args[0] === "clean") {
    if (!userResorts.has(senderID)) {
      return message.reply(
        "𝚈𝚘𝚞 𝚍𝚘𝚗'𝚝 𝚘𝚠𝚗 𝚊 𝚛𝚎𝚜𝚘𝚛𝚝. 𝚄𝚜𝚎 `resort buy` 𝚝𝚘 𝚙𝚞𝚛𝚌𝚑𝚊𝚜𝚎 𝚘𝚗𝚎"
      );
    }
    ...
    if (currentTime - lastCleanTime < cleanlinessCooldownDuration) {
      const remainingCooldown =
        cleanlinessCooldownDuration - (currentTime - lastCleanTime);
      const remainingCooldownHours = Math.ceil(
        remainingCooldown / (60 * 60 * 1000)
      );

      return message.reply(
        `𝚈𝚘𝚞𝚛 𝚛𝚎𝚜𝚘𝚛𝚝 𝚒𝚜 𝚊𝚕𝚛𝚎𝚊𝚍𝚢 𝚌𝚕𝚎𝚊𝚗. 𝙸𝚝 𝚌𝚊𝚗 𝚋𝚎 𝚌𝚕𝚎𝚊𝚗𝚎𝚍 𝚊𝚐𝚊𝚒𝚗 𝚒𝚗 ${remainingCooldownHours} 𝚑𝚘𝚞𝚛. ⏱`
      );
    }
    ...
    return message.reply(
      "𝚈𝚘𝚞'𝚟𝚎 𝚌𝚕𝚎𝚊𝚗𝚎𝚍 𝚢𝚘𝚞𝚛 𝚛𝚎𝚜𝚘𝚛𝚝! 𝙸𝚝'𝚜 𝚗𝚘𝚠 𝚜𝚙𝚊𝚛𝚔𝚕𝚒𝚗𝚐 𝚌𝚕𝚎𝚊𝚗. 🧹"
    );
  }
  ...
  if (args[0] === "upgrade") {
    ...
    const timeSinceLastUpgrade =
      currentTime - (userResort.lastUpgradeTime || 0);
    ...
    if (timeSinceLastUpgrade < upgradeCooldownDuration) {
      ...
      return message.reply(
        `𝚈𝚘𝚞 𝚌𝚊𝚗'𝚝 𝚞𝚙𝚐𝚛𝚊𝚍𝚎 𝚢𝚘𝚞𝚛 𝚛𝚎𝚜𝚘𝚛𝚝 𝚢𝚎𝚝. 𝙿𝚕𝚎𝚊𝚜𝚎 𝚠𝚊𝚒𝚝 𝚏𝚘𝚛 ${remainingCooldownHours} 𝚑𝚘𝚞𝚛𝚜 𝚋𝚎𝚏𝚘𝚛𝚎 𝚞𝚙𝚐𝚛𝚊𝚍𝚒𝚗𝚐 𝚊𝚐𝚊𝚒𝚗. ⏱`
      );
    }
    ...
    if (userBalance < upgradePrice) {
      return message.reply(
        `𝚈𝚘𝚞 𝚍𝚘𝚗'𝚝 𝚑𝚊𝚟𝚎 𝚎𝚗𝚘𝚞𝚐𝚑 𝚌𝚛𝚎𝚍𝚒𝚝𝚜 𝚝𝚘 𝚞𝚙𝚐𝚛𝚊𝚍𝚎 𝚢𝚘𝚞𝚛 𝚛𝚎𝚜𝚘𝚛𝚝. 𝚃𝚑𝚎 𝚞𝚙𝚐𝚛𝚊𝚍𝚎 𝚌𝚘𝚜𝚝𝚜 ${upgradePrice}.`
      );
    }
    ...
    const { value: nextValue, imageURL: nextImageURL } =
      calculateResortValue(nextLevel);
    ...
    if (nextLevel === maxResortLevel) {
      return message.reply(
        upgradeSuccessMessage + "𝚈𝚘𝚞𝚛 𝚛𝚎𝚜𝚘𝚛𝚝 𝚒𝚜 𝚗𝚘𝚠 𝚊𝚝 𝚝𝚑𝚎 𝚖𝚊𝚡𝚒𝚖𝚞𝚖 𝚕𝚎𝚟𝚎𝚕!"
      );
    }
  ...
  if (args[0] === "collect") {
    ...
    const collectMessage = `𝚈𝚘𝚞'𝚟𝚎 𝚌𝚘𝚕𝚕𝚎𝚌𝚝𝚎𝚍 ₱${collectedAmount} 𝚟𝚊𝚕𝚞𝚎 𝚏𝚛𝚘𝚖 𝚢𝚘𝚞𝚛 𝚛𝚎𝚜𝚘𝚛𝚝! 💰`;
  ...
  if (args[0] === "rename") {
    ...
    return message.reply(
      `𝚈𝚘𝚞𝚛 𝚛𝚎𝚜𝚘𝚛𝚝 𝚑𝚊𝚜 𝚋𝚎𝚎𝚗 𝚛𝚎𝚗𝚊𝚖𝚎𝚍 𝚝𝚘 "${newResortName}". 🏨`
    );
  }
